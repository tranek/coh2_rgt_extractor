...
CFileMap::_File* CFileMap::_FindFile(const char* sName, CFileMap::_Folder** ppFolder, bool bIsFolder)
{
	...
	for(std::vector<_TOC*>::iterator itr = m_vTOCs.begin(); itr != m_vTOCs.end(); ++itr)
	{
		if( (strlen((**itr).sName) == iPartLen) &&  (_strnicmp((**itr).sName, sName, iPartLen) == 0) )
		{
			pToc = *itr;
			break;
		}
	...
	while(sSlashLoc)
	{
		...
		{
			int iNiRes = _strnicmp(pFolder->vChildFolders[iBSM]->sName, sNameLeft, iPartLen);
			if(iNiRes > 0)
			{
				iBSH = iBSM;
				iBSM = (iBSL + iBSH) >> 1;
	...
	while(iBSH > iBSL)
	{
		int iNiRes = _stricmp(pFolder->vChildFiles[iBSM]->sName, sNameLeft);
		if(iNiRes > 0)
		{
			iBSH = iBSM;
			iBSM = (iBSL + iBSH) >> 1;
...
{
	return (_stricmp(a->sName, b->sName) < 0);
}
...
void CFileMap::MapSingleFile(void* pSource, const char* sTocName, const char* sFullPath, const char* sPathPartial)
{
	...
	for(std::vector<_TOC*>::iterator itr = m_vTOCs.begin(); itr != m_vTOCs.end(); ++itr)
	{
		if(_stricmp((**itr).sName, sTocName) == 0)
		{
			pToc = *itr;
			break;
		}
	...
	while(sPathSeperator)
	{
		...
		{
			if(strlen((**itr).sName) == iPartL && _strnicmp((**itr).sName, sPathPartial, iPartL) == 0)
			{
				pTheFolder = *itr;
				break;
			}
	...
	{
		if(_stricmp(sPathPartial, (**itr).sName) == 0)
		{
			pTheFile = *itr;
			break;
		}
...
void CFileMap::MapIteratorDeep(void* pSource, const char* sPath, IDirectoryTraverser::IIterator* pItr)
{
	...
	for(std::vector<_TOC*>::iterator itr = m_vTOCs.begin(); itr != m_vTOCs.end(); ++itr)
	{
		if(strlen((**itr).sName) == iPartLen && _strnicmp((**itr).sName, sPath, iPartLen) == 0)
		{
			pToc = *itr;
			break;
		}
	...
	while(pItr->VGetType() != IDirectoryTraverser::IIterator::T_Nothing)
	{
		if(pItr->VGetType() == IDirectoryTraverser::IIterator::T_File)
		{
			if(!pSource->bIsPureOutput)
			{
				...
				for(std::vector<_File*>::iterator itr = pFolder->vChildFiles.begin(); itr != pFolder->vChildFiles.end(); ++itr)
				{
					if(_stricmp((**itr).sName, pItr->VGetName()) == 0)
					{
						pTheFile = *itr;
						break;
					}
