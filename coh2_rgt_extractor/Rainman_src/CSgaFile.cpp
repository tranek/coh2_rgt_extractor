...
IFileStore::IStream* CSgaFile::VOpenStream(const char* sIdentifier)
{
	...
	*/
	...
	else
	{
		for(unsigned short i = 0; i < m_pDataHeaderInfo->iToCCount; ++i)
		{
			...
			*/
			if(strlen(m_pSgaToCs[i].sAlias) == iToCLength && _strnicmp(m_pSgaToCs[i].sAlias, sIdentifier, iToCLength) == 0)
			{
				iToC = i;
				break;
			}
	...
	*/
	...
	while(iBSH > iBSL)
	{
		int iNiRes = _stricmp(m_pSgaFileExts[iBSM].sName, sFileNameBegin);
		if(iNiRes > 0)
		{
			iBSH = iBSM;
			iBSM = (iBSL + iBSH) >> 1;
...
IDirectoryTraverser::IIterator* CSgaFile::VIterate(const char* sPath)
{
	...
	*/
	...
	for(unsigned short i = 0; i < m_pDataHeaderInfo->iToCCount; ++i)
	{
		/*
			...
			then Data and Data2 could match each other. If strlen is included then they do not match.
		*/
		if(strlen(m_pSgaToCs[i].sAlias) == iPartLength && _strnicmp(m_pSgaToCs[i].sAlias, sPath, iPartLength) == 0)
		{
			iToC = i;
			break;
		}
	...
	*/
	...
	while(iPartLength != 0)
	{
		...
		{
			if(strlen(m_pSgaDirExts[i].sShortName) == iPartLength && _strnicmp(m_pSgaDirExts[i].sShortName, sPath, iPartLength) == 0)
			{
				iSubDir = i;
				break;
			}
